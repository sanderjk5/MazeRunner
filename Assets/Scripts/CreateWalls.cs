...
public class CreateWalls : MonoBehaviour
{
    ...
    void CreateAllWalls()
    {
        ...
        {
            Instantiate(wallPrefab, new Vector3(i + 0.5f, height / 2, 0), Quaternion.identity);
            Instantiate(wallPrefab, new Vector3(i + 0.5f, -height / 2, 0), Quaternion.identity);
        }
        for (int i = -height / 2; i < height / 2; i++)
        {
            Instantiate(wallPrefab, new Vector3(-width / 2, i + 0.5f, 0), Quaternion.Euler(0, 0, 90));
            Instantiate(wallPrefab, new Vector3(width / 2, i + 0.5f, 0), Quaternion.Euler(0, 0, 90));
        }

        for (int i = -width / 2; i < width / 2; i++)
        {
            int lastNode = (i + width / 2) * height;
